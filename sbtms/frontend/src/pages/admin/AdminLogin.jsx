...
export default function AdminLogin() {
  ...
  const [form, setForm] = useState({ email: 'admin@sbtms.com', password: 'admin123' });
  const [err, setErr] = useState(''); const [busy, setBusy] = useState(false);
  const submit = async (e) => {
    e.preventDefault(); setErr(''); setBusy(true);
    try { await adminLogin(form.email, form.password); nav('/admin'); }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800 px-4">
      <form onSubmit={submit} className="card max-w-md w-full bg-white/95 backdrop-blur-sm">
        ...
        <h1 className="text-2xl font-bold mb-1 text-slate-800">Admin Sign in</h1>
        <p className="text-sm text-slate-500 mb-6">SBTMS Administration Portal</p>
